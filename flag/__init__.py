...
class _Flag(ABC):
    ...
    def value(self, value: Any) -> None:
        try:
            ...
            _write_exc(f'Cannot convert value "{value}" for {type(self).__name__} "{self.name}". (expected type: {self.type.__name__})')
...
def parse() -> None:
    ...
    if len(_unsatisfied_mandatory) != 0:
        _write_exc(f'Mandatory flag "{list(_unsatisfied_mandatory.values())[0].name}" not provided.')